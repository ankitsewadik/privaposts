		...
		function _add_rewrite_rules( $rules ) {
			...
			if ( isset( UM()->config()->permalinks['user'] ) ) {
				...
				if ( isset( $user->post_name ) ) {
					...
					$trimmed_link = rtrim($actual_link,'/');
					$link_array = explode('/', $trimmed_link);
					$page_slug = end($link_array);
					$page = get_page_by_path( $page_slug );
				
				//	exit;
					if(empty($page)){
						$newrules[ '([^/]+)/?$' ] = 'index.php?page_id='.$user_page_id.'&um_user=$matches[1]&lang='.$add_lang_code;
					}
				}
			}
