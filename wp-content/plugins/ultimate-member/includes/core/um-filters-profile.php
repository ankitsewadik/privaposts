...
function um_change_profile_cover_photo_label( $args ){
	...
	if( $file_size >= 999999999  ){
			...
			$args['cover_photo']['upload_text'] .= '<small class=\'um-max-filesize\'>( 1600 x 400 )</small>';
		}
		return $args;
	}
...
function um_change_profile_photo_label( $args ) {
	...
	if ( $file_size < 999999999 ) {
		$args['profile_photo']['upload_text'] .= '<small class=\'um-max-filesize\'>( '.__('max','ultimate-member').': <span>10'.$unit.'</span> )</small>';
		//$args['profile_photo']['upload_text'] .=  '<small class=\'um-max-filesize\'>( 300 x 300 )</small>';
	}
	return $args;
}
